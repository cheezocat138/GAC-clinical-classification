...
def load_model(model_path=MODEL_PATHS['ensemble']):
    ...
    if 'fold_models' in checkpoint:
        ...
        if selected_indices is None or len(selected_indices) == 0:
            ...
            selected_indices = list(range(min(500, 20000)))  # 使用前500个特征，防止超出内存
        ...
        if selected_indices is None or len(selected_indices) == 0:
            print("没有特征选择信息，将使用所有特征")
            selected_indices = list(range(min(500, 20000)))  # 使用前500个特征
...
def predict_survival_group(data, model_info, return_probabilities=False):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        if model_info['selected_features'] and len(model_info['selected_features']) > 0:
            ...
            if not selected_features:
                print("警告：没有可用的特征进行预测，将使用所有特征的前500个")
                if data.shape[1] > 500:
                    # 对于大规模数据，只使用前500个特征
                    features = data.iloc[:, :500].values
                else:
            ...
            if not valid_indices:
                print("警告：没有有效的特征索引进行预测，将使用前500个特征")
                if data.shape[1] > 500:
                    features = data.iloc[:, :500].values
                else:
                    features = data.values
            ...
            # 没有特征选择信息，对于大规模数据只使用前500个特征
            if data.shape[1] > 500:
                print("没有特征选择信息，对于大规模数据只使用前500个特征")
                features = data.iloc[:, :500].values
            else:
    ...
    if features.shape[1] != model_info['input_size']:
        print(f"特征数量不匹配: 当前={features.shape[1]}, 模型期望={model_info['input_size']}")
        if features.shape[1] > model_info['input_size']:
            # 如果特征太多，只使用前面的部分
            print("特征数量过多，截取所需部分...")
            features = features[:, :model_info['input_size']]
        else:
            # 如果特征不足，用零填充
            print("特征数量不足，用零填充...")
            pad_width = model_info['input_size'] - features.shape[1]
...
def predict_from_rppa(rppa_file=DATA_PATHS['default_transposed'], model_path=MODEL_PATHS['ensemble'], output_file=PREDICTION_PATH):
    """从RPPA数据文件预测生存组别"""
    # 加载RPPA数据
    print(f"加载数据文件: {rppa_file}")
    ...
    print(f"数据形状: {rppa_features.shape}")
